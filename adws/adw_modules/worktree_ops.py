...
def cleanup_worktree_and_state(
    ...
    # Clean up lock file before archiving
    state_dir = agents_base / adw_id
    if state_dir.exists():
        lock_file = state_dir / ".adw_state.lock"
        if lock_file.exists():
            try:
                lock_file.unlink()
                logger.info(f"Removed lock file for {adw_id}")
            except OSError as e:
                logger.warning(f"Failed to remove lock file for {adw_id}: {e}")
