...
class ClaudeClient(ProviderClient):
    """
    Claude Opus client (The Architect).

    Model: claude-opus-4-5-20251101 (configurable via TRINITY_ARCHITECT_MODEL env var)
    ...
    def __init__(self, api_key: str | None = None) -> None:
        ...
        env_model = os.getenv("TRINITY_ARCHITECT_MODEL", "claude-opus-4-5-20251101")
    ...
    async def complete(
        ...
        """
        Generate completion using Claude with fallback chain.
        ...
        for model_name in models_to_try:
            try:
                ...
                response = await asyncio.wait_for(
                    self.client.messages.create(**create_kwargs),  # type: ignore[arg-type]
                    timeout=timeout,
                )
        ...
        raise RuntimeError(
            f"All Claude models in fallback chain failed. Last error: {last_error}"
        )
...
class GPTClient(ProviderClient):
    ...
    def __init__(self, api_key: str | None = None) -> None:
        ...
        env_model = os.getenv("TRINITY_CRITIC_MODEL", "gpt-5.2")
        if env_model not in self.FALLBACK_CHAIN:
            self._fallback_models = [env_model] + self.FALLBACK_CHAIN
        else:
            self._fallback_models = self.FALLBACK_CHAIN.copy()
...
class GeminiClient(ProviderClient):
    """
    Gemini 3.1 Pro client (The Advocate).

    Model: gemini-3.1-pro-preview (configurable via env)
    Note: gemini-3-flash-preview deprecated March 9, 2026
    """
    ...
    def __init__(self, api_key: str | None = None) -> None:
        ...
        self.model_name = os.getenv("TRINITY_ADVOCATE_MODEL", "gemini-3.1-pro-preview")
